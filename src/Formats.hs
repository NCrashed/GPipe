...
instance (bool ~ BooleanOf a, IfB a) => IfB (Color AlphaFormat a) where
    ifB c (Alpha t) (Alpha e) = Alpha (ifB c t e)
instance (bool ~ BooleanOf a, IfB a) => IfB (Color LuminanceFormat a) where
    ifB c (Luminance t) (Luminance e) = Luminance (ifB c t e)
instance (bool ~ BooleanOf a, IfB a) => IfB (Color LuminanceAlphaFormat a) where
    ifB c (LuminanceAlpha t1 t2) (LuminanceAlpha e1 e2) = LuminanceAlpha (ifB c t1 e1) (ifB c t2 e2)
instance (bool ~ BooleanOf a, IfB a, IfB (Vec3 a)) => IfB (Color RGBFormat a) where
    ifB c (RGB t) (RGB e) = RGB (ifB c t e)
instance (bool ~ BooleanOf a, IfB a, IfB (Vec3 a)) => IfB (Color RGBAFormat a) where
